...
import (
	...
	"gestrym-nutrition/src/common/routes"
...
func setupEnvironment(isLocalEnv bool) error {
	if isLocalEnv {
		logger.Info("[GESTRYM_NUTRITION] server started in development mode")
		...
		logger.Info("[GESTRYM_NUTRITION] server started in production mode")
...
func initServer() {
	address := viper.GetString("GESTRYM_NUTRITION_SERVER_ADDRESS")
	ginMode := viper.GetString("GIN_MODE")
	if address == "" {
		logger.Fatal("Server address env 'GESTRYM_NUTRITION_SERVER_ADDRESS' not set")
	}
	if ginMode == "" {
	...
	logger.Info("[GESTRYM_NUTRITION] Start server on -> %v", address)
